      ...
      {attendance.slice(1).map((record) => (
        <Card key={record.id} style={styles.historyCard}>
          <View style={styles.historyRow}>
            ...
            <StatusBadge label={record.status} tone={record.status === 'present' ? 'success' : 'neutral'} />
          </View>
          <Text style={[styles.historyDetail, { color: colors.textSecondary }]}>
            {record.punchIn?.slice(0, 5) ?? '—'} → {record.punchOut?.slice(0, 5) ?? '—'}
            {record.wifiSsid ? ` · ${record.wifiSsid}` : ''}
          </Text>
        </Card>
      ))}
    </ScrollView>
...
const styles = StyleSheet.create({
  ...
  punchTime: { fontSize: 28, fontWeight: '700' },
