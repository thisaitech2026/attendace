...
export default function PunchModal() {
  ...
  const [lastPunchDetail, setLastPunchDetail] = useState<string[] | null>(null);
  ...
  useEffect(() => {
    if (today?.punchIn) {
      setLastPunchDetail(formatPunchDetailLines(today, employeeName));
    }
  }, [today, employeeName]);

  const showPunchResult = (record: typeof today) => {
    if (!record) return;
    const lines = formatPunchDetailLines(record, employeeName);
    setLastPunchDetail(lines);
  ...
  return (
    ...
    >
      ...
      <Text style={[styles.subtitle, { color: colors.textSecondary }]}>
        Review your attendance and punch in or out
      </Text>
      ...
      {lastPunchDetail ? (
        <Card style={[styles.detailCard, { borderColor: colors.primary, backgroundColor: colors.primaryLight }]}>
          <Text style={[styles.detailTitle, { color: colors.primary }]}>Latest punch record</Text>
          {lastPunchDetail.map((line) => (
...
const styles = StyleSheet.create({
  ...
  subtitle: { fontSize: 14, marginTop: 6, marginBottom: 16, fontWeight: '500' },
  ...
  detailCard: { marginBottom: 14, borderWidth: 1, gap: 4 },
  detailTitle: { fontSize: 14, fontWeight: '800', marginBottom: 4 },
